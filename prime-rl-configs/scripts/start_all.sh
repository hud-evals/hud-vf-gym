...
tmux send-keys -t "$SESSION:0.1" "bash -lc 'set -o pipefail; cd \"$ROOT\"; LOG_FILE=$ORCH_LOG; { echo \"====\"; date +\"%Y-%m-%dT%H:%M:%S%z\"; echo \"Starting orchestrator\"; } >> \$LOG_FILE; uv run orchestrator @ \"$HUD_CONFIG_DIR/orch.toml\" 2>&1 | tee -a \$LOG_FILE'" C-m

# Pane 2: trainer on GPU1
tmux split-window -h -t "$SESSION:0.1"
tmux send-keys -t "$SESSION:0.2" "bash -lc 'set -o pipefail; cd \"$ROOT\"; LOG_FILE=$TRAIN_LOG; { echo \"====\"; date +\"%Y-%m-%dT%H:%M:%S%z\"; echo \"Starting trainer\"; } >> \$LOG_FILE; export CUDA_VISIBLE_DEVICES=1; uv run trainer @ \"$HUD_CONFIG_DIR/rl.train.toml\" 2>&1 | tee -a \$LOG_FILE'" C-m
